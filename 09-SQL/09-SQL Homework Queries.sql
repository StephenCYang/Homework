...
select *
	...
	order by hire_date desc; -- I also did an ascending order one just to make sure it started from Jan 1.
...
select dm.dept_no, d.dept_name, dm.emp_no, e.last_name, e.first_name, e.hire_date from dept_manager as dm
	...
	order by dm.dept_no, hire_date;

-- Hey! There's no end employment date provided. The dates are just hire dates. There's no reason to assume they
-- were hired in to replace the previous manager on that date, or that they were promoted into the position. There's
-- no way to tell an end employment date.
